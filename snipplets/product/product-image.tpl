...
{% if product.images_count > 0 %}
	<div class="js-swiper-product swiper-container" style="visibility:hidden; height:0;">
	    ...
	    {% if has_multiple_slides %}
		    <div class="js-swiper-product-prev swiper-button-prev d-none d-md-block">{% include "snipplets/svg/chevron-left.tpl" with {svg_custom_class: "icon-inline icon-w-16 icon-2x svg-icon-text"} %}</div>
	        <div class="js-swiper-product-next swiper-button-next d-none d-md-block">{% include "snipplets/svg/chevron-right.tpl" with {svg_custom_class: "icon-inline icon-w-16 icon-2x svg-icon-text"} %}</div>
        {% endif %}
	</div>
