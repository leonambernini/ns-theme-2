...
$(document).ready(function(){
	...
	{% if template == 'home' %}
        ...
        {% if sections.primary.products or sections.secondary.products or sections.products_month.products %}
            ...
            {% set grid_mobile_one = settings.grid_mobile_one %}
            var featuredSwiper = new Swiper ('.js-swiper-featured', {
                ...
                slidesPerView: {% if columns == 2 and not grid_mobile_one %}2{% else %}1{% endif %},
                pagination: {
            ...
            var featuredSwiper2 = new Swiper ('.js-swiper-featured-2', {
                lazy: true,
                loop: true,
                spaceBetween: 10,
                watchSlidesVisibility: true,
                ...
                slidesPerView: {% if columns == 2 and not grid_mobile_one %}2{% else %}1{% endif %},
                pagination: {
                    el: '.js-swiper-featured-2-pagination',
                    clickable: true,
                },
                navigation: {
                    nextEl: '.js-swiper-featured-2-next',
                    prevEl: '.js-swiper-featured-2-prev',
                },
                breakpoints: {
                    640: {
                        slidesPerView: 2,
                ...
                {% if settings.product_color_variants or settings.quick_shop %}
                    on: {
                        init: function () {
                            updateClonedItemsIDs(".js-swiper-featured-2 .js-item-slide.swiper-slide-duplicate");
                        },
                    }
                {% endif %}
            });

            var featuredSwiper3 = new Swiper ('.js-swiper-featured-3', {
                lazy: true,
                loop: true,
                spaceBetween: 10,
                watchSlidesVisibility: true,
                ...
                slidesPerView: {% if columns == 2 and not grid_mobile_one %}2{% else %}1{% endif %},
                pagination: {
                    el: '.js-swiper-featured-3-pagination',
                    clickable: true,
                },
                navigation: {
                    nextEl: '.js-swiper-featured-3-next',
                    prevEl: '.js-swiper-featured-3-prev',
                },
                breakpoints: {
                    640: {
                        slidesPerView: 3,
                ...
                {% if settings.product_color_variants or settings.quick_shop %}
                    on: {
                        init: function () {
                            updateClonedItemsIDs(".js-swiper-featured-3 .js-item-slide.swiper-slide-duplicate");
                        },
                    }
                {% endif %}
            });
    ...
    {% if settings.custom_fields %}
        if( $('.js-custom-field-file-cta').length ){
            $('.js-custom-field-file-cta').click( function(){
                $('.js-custom-field-file').click();
                return false;
            });
            ...
            function readURL(input) {
                if (input) {
                    ...
                    reader.onload = function (e) {
                        $('.custom-field-file-preview-thumb').html('<img src="'+e.target.result+'" class="custom-field-file-preview-thumb-img" style="max-height: 100px;" />');
                        $('.custom-field-file-mokup-preview').attr('src', e.target.result);
                    }
            ...
            var CUSTOM_FILE_UPLOAD_TEMP = null;

            {% if settings.custom_fields_dropbox_token %}
                $('.custom-field-file-mokup-cta-edit').click( function(){
                    $('.custom-field-file-mokup').addClass('edit-true');
                    return false;
                });
                $('.custom-field-file-mokup-cta-save').click( function(){
                    $('.custom-field-file-mokup').removeClass('edit-true');
                    return false;
                });
                
                $('.custom-field-file-mokup .js-close-popup').click( function(){
                    $('.custom-field-file-mokup').fadeOut();
                    return false;
                });

                $('.js-addtocart').click( function(){
                    var fileName = CUSTOM_FILE_UPLOAD_TEMP.name;
                    var settings = {
                        ...
                        processData: false,
                        beforeSend: function(){
                            $('.js-addtocart').prop("disabled",true);
                        },
                        headers: {
                            "Content-Type": "application/octet-stream",
                            "Authorization": "Bearer {{ settings.custom_fields_dropbox_token }}",
                            "Dropbox-API-Arg": "{\"path\":\"/"+(Math.floor(Math.random() * 26) + Date.now()).toString()+"__"+fileName+"\",\"mode\":\"add\",\"autorename\":true,\"mute\":false}"
                        },
                        data: CUSTOM_FILE_UPLOAD_TEMP,
                    };
                        
                    $.ajax(settings).done(function (response) {
                        if( response == undefined || response == null ){
                            $('.js-custom-field-file-cta').after('<div class="alert-danger">Não conseguimos fazer upload da sua imagem, tente novamente.</div>');
                            return false;
                        }
                        if( response.name ){
                            $('.js-custom-field-file-name').val(response.name);
                        }
                        console.log(response);
                        $('.js-addtocart').prop("disabled",false);
                    });
                });

                $('.js-custom-field-file').change( function(e){

                    if( $('.custom-field-file-preview-thumb').length ){
                        readURL(e.target.files[0])
                    }

                    CUSTOM_FILE_UPLOAD_TEMP = e.target.files[0];

                    if( $('.custom-field-file-mokup').length ){
                        var $img = $('.custom-field-file-mokup-image');
                        $img.attr('src', $img.attr('data-src'));
                        $('.custom-field-file-mokup').fadeIn(function(){
                            $('#mokupW').text('Mokup Width: ' + ($('.custom-field-file-mokup-image').width()) + 'px');
                            $('#mokupH').text('Mokup Height: ' + ($('.custom-field-file-mokup-image').height()) + 'px');
                            $('.js-custom-field-positions-mokup').val(($('.custom-field-file-mokup-image').width()) + 'px' + ($('.custom-field-file-mokup-image').height()) + 'px')
                            ...
                            $('#draggableHelper').draggable({
                                drag: function() {
                                    ...
                                    var contentWidth = $('.custom-field-file-mokup-content').width();
                                    var windowHeight = $(window).height();
                                    var contentHeight = $('.custom-field-file-mokup-content').height();
                                    
                                    xPos = (xPos - ((windowWidth-contentWidth)/2));
                                    yPos = (yPos - ((windowHeight-contentHeight)/2));
                                    $('#posX').text('x: ' + xPos + 'px');
                                    $('#posY').text('y: ' + yPos + 'px');

                                    $('.js-custom-field-positions-image-pos').val('x = ' + xPos + 'px | y = ' + yPos + 'px');
                                }
                            });
                            $(".custom-field-file-mokup-preview").resizable({
                                aspectRatio: true,
                                resize: function( event, ui ) {
                                    $('#imageW').text('Image Width: ' + ($(this).width()) + 'px');
                                    $('#imageH').text('Image Height: ' + ($(this).height()) + 'px');

                                    $('.js-custom-field-positions-image').val($(this).width() + 'px' + $(this).height() + 'px');
                                }
                            });
                        });
                    }
